...
parts.append(label('【题目】'))
parts.append(p_text('同一带正电粒子均从正极板旁由静止释放，不计重力。以板间电压为U₀、板间距为d₀的方案A作为参照，粒子所受静电力为F₀，到达负极板时的速度为v₀。各方案参数如下表。'))
parts.append(table([
    [hei('方案', size=22), hei('U/U₀', size=22), hei('d/d₀', size=22), hei('E/E₀', size=22), hei('F/F₀', size=22), hei('v/v₀', size=22)],
...
parts.append(p_text('根据动能定理qU=mv²/2可得'))
parts.append(eq(ratio(v(), v('0')) + mop('=') + mrad(ratio(sym('U'), sym('U', '0')))))
parts.append(p_text('代入各方案的U/U₀和d/d₀，即可得到表中各比值。该结果表明，改变板间距会改变加速度和运动时间，但在电压一定时不会改变末动能。'))
...
parts.append(p_runs([cn('C．粒子在两板间的加速度为 '), phys('a'), roman('='), roman('2'), variable_with_sub('v', '0'), roman('²/'), phys('L')]))
...
parts.append(p_text('因此C项正确。第二个粒子从N板下端射出的实际时间为L/(2v₀)，D项所给时间是第一个粒子在水平方向加速所需的时间，故D项错误。'))
...
with ZipFile(OUT) as z:
    ...
    # 关键结果一致性
    for text in ['40 m/s', '30 m/s', '28.3 m/s', 'A、C、D三方案静电力相同', 'L/(2v₀)']:
        assert text in xml
