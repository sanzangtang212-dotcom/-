...
add_label(doc,'【答案】')
add_body(doc,'（1）R2；（2）电流表串联，电压表测路端电压，滑动变阻器采用限流接法；（3）电动势基本不变，内阻减小；（4）约283 Ω。')
add_label(doc,'【解析】')
...
add_body(doc,'因此')
add_formula(doc,r()+mu('=')+mfrac(E()+mu('−')+U(),I())+mu('=')+mfrac(mu('1.17−1.00'),mu('0.60×')+msup(mu('10'),mu('−3')))+mu(' Ω')+mu('≈283 Ω'))
