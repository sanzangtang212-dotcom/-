...
add_body(doc, '注意：电动机正常转动时是非纯电阻用电器，不能用U²/R或UI=I²R计算其总功率。')
...
add_label(doc, '【题目】')
add_body(doc, '如图，电源电压恒为220 V，保护电阻R=10 Ω，电动机线圈电阻r=1.0 Ω。电动机正常工作时，电压表测得电动机两端电压为170 V。电动机匀速提升质量为10 kg的重物，不计摩擦，g取10 m/s²。')
...
add_formula(doc, v() + mu('=') + mfrac(mu('825 W'), mdelim(mu('10 kg×10 m/s²'))) + mu('=8.25 m/s'))
...
add_label(doc, '【题目】')
add_body(doc, '某节水喷灌系统中，水以15 m/s的速度水平喷出，每秒喷出水的质量为2.0 kg。喷出的水从井下抽取，喷口离水面的高度保持3.75 m不变。水泵由电动机带动，电动机正常工作时输入电压为220 V，输入电流为2.0 A。不计电动机的摩擦损耗，电动机的输出功率等于水泵的输入功率。水泵效率为75%，忽略水在管道中运动的机械能损失，g取10 m/s²。下列说法正确的是（　　）')
